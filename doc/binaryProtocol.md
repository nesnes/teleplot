...
`{PROTOCOL_VERSION[uint8]} - {PACKET_ID[uint8]} - {DATA[x]} - {CHECKSUM[16]}`

  - **PROTOCOL_VERSION** represent a fixed definition of this protocol and is here for potential future compatibility needs.
  - **PACKET_ID** in an uint8 that gets incremented for every new packet, and that overflows. It is used to estimate connection quality as a simple way of detecting lost packets.
  - **DATA** is not fixed in size and organized as a list of typed SECTION.
  - **CHECKSUM**: checksum of the `{PROTOCOL_VERSION[uint8]} - {PACKET_ID[uint8]} - {DATA[x]}` section to ensure data consistency. (**TODO** define which kind of checksum to use! -> just check no reconstruction)
...
```
 -------------------------------------------------------------------------------------------PACKET---------------------------------------------------------------------------------------------------------------
 PROTOCOL_VERSION   PACKET_ID   -------------------------------------------------------------DATA-----------------------------------------------------------------------------------------------------   CHECKSUM
 ---------------------------------------------------------------------------------------HASHED BYTES--------------------------------------------------------------------------------------------------
                                --------------------SECTION 1 (TELEM_ATTR)-------------------------   --------------------------------------SECTION 2 (TELEM_DATA_NUMBER)-----------------------------
                                               ----------------------------SECTION_DATA------------                  ----------TELEM_DATA_HEADER------   -TELEM_DATA_NUMBER 1-   -TELEM_DATA_NUMBER 2-
 PROTOCOL_VERSION - PACKET_ID - SECTION_TYPE - TELEM_ID - COUNT - TELEM_ATTRCODE -  TELEM_ATTR_DATA - SECTION_TYPE - TELEM_ID - TIME_REFERENCE - COUNT -  TIMEDIFF  -  VALUE   -  TIMEDIFF  -  VALUE   - CHECKSUM
         1              42           1            300       1            0         't' 'e' 'm' 'p' 0       2            300      105600584045      2       1200        21.0         1300       21.5         12
```
...
Here is the list of the defined **SECTION_TYPE** and their specifications:
  ...
  - `1` **TELEM_ATTR**: Definition of telemetry attributes.
    - **SECTION_DATA**: `{TELEM_ID[16]} - {COUNT[8]} - TELEM_ATTR({TELEM_ATTR_CODE[8]} - {TELEM_ATTR_DATA})...`
      ...
      - List of **TELEM_ATTR**s, with their code and data description:  `code` **MY_ATTR**(data size)
        ...
        - `5` **TELEM_ATTR_SHAPE** `{TELEM_ATTR_SHAPE_TYPE[8] - TELEM_ATTR_SHAPE_DATA[x]}`:
          - `6` **TELEM_ATTR_SHAPE_CUBE**: the 2D object is a square, and the 3D object is a cube. (no **TELEM_ATTR_SHAPE_DATA**)
          - `7` **TELEM_ATTR_SHAPE_SPHERE**: the 2D object is a circle, and the 3D object is a sphere. (no **TELEM_ATTR_SHAPE_DATA**)
          - `8` **TELEM_ATTR_SHAPE_CYLINDER**: the 3D object is a cylinder. (no **TELEM_ATTR_SHAPE_DATA**)
          - `9` **TELEM_ATTR_SHAPE_STL**: the 3D object is defined by an STL file. **TELEM_ATTR_SHAPE_DATA**(null terminated string): the URL to the STL file. *NOTE: urls needs to be accessible by the viewer (so online or locally served with proper CORS policies)*
  ...
  - `7` **TELEM_DATA_SHAPE_3D_POSITION**:
  ...
  - `8` **TELEM_DATA_SHAPE_3D_ROTATION**: *Prefer TELEM_DATA_SHAPE_3D_QUATERNION to set shape orientation*
  ...
  - `9` **TELEM_DATA_SHAPE_3D_QUATERNION**:
